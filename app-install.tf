resource "null_resource" "app" {
 //   triggers = {ver = var.APP_VERSION} // Whenever these is a change in the versio only during that time it will run.
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "ansible-pull -U https://github.com/23-Pravallika/Ansible.git robo-pull.yml -e DOCDB_ENDPOINT=${data.terraform_remote_state.db.outputs.DOCDB_ENDPOINT} -e ENV=dev -e COMPONENT=${var.COMPONENT} -e APP_VERSION=${var.APP_VERSION}"
    ]
  }
}
